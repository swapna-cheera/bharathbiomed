...
class ManageDesignationsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ManageDesignationsScreen> createState() =>
      _ManageDesignationsScreenState();
}

class _ManageDesignationsScreenState
    extends ConsumerState<ManageDesignationsScreen> {
  ...
  Future<void> _runGuarded(Future<void> Function() action,
    ...
    debugPrint(
        'ManageDesignationsScreen._runGuarded: running action "$failureTitle"');
    try {
      await action();
      debugPrint(
          'ManageDesignationsScreen._runGuarded: action succeeded "$failureTitle"');
    } catch (error, stackTrace) {
      debugPrint(
          'ManageDesignationsScreen._runGuarded: action failed "$failureTitle" error=$error');
      AppLogger.error('ManageDesignations', failureTitle,
  ...
  Future<void> _delete(Designation designation) async {
    debugPrint(
        'ManageDesignationsScreen._delete: delete requested id=${designation.id}');
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Delete designation?'),
        content: Text(
            '"${designation.name}" will no longer be selectable for new employees.'),
        actions: [
          TextButton(
    ...
    await _runGuarded(
      () => ref
          .read(designationControllerProvider.notifier)
            ...
            Expanded(
              child: designations.when(
                ...
                data: (items) {
                  ...
                  if (filtered.isEmpty) {
                    return const Center(
                        child: Text('No designations match this search.'));
                  }
                  return RefreshIndicator(
                    onRefresh: () =>
                        ref.refresh(designationControllerProvider.future),
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          subtitle: Text(
                              '${designation.category.label} · Level ${designation.hierarchyLevel}'),
                          trailing: Row(
